...
let lastHeatPumpReport = null;

async function renderHeatPumpLegacy() {
  ...
  document.querySelector("#content").innerHTML = `<section class="section"><h2>Wärmepumpenstatus</h2><div class="card"><div class="room-name">${esc(report.model)} · ${report.online ? "Online" : "Offline"}</div><div class="room-detail">Betriebsmodus ${esc(metrics.operating_mode)} · Verdichter ${metrics.compressor_active ? "aktiv" : "bereit"}</div></div></section><section class="section"><h2>Speichertemperaturen · letzte 24 Stunden</h2><div class="storage-history-grid"><div class="card dhw-history-card">${storageHistoryChart(report.dhw_history, "Warmwasserspeicher", "#d97706", "#f59e0b")}</div><div class="card dhw-history-card">${storageHistoryChart(report.buffer_history, "Pufferspeicher", "#2563eb", "#60a5fa")}</div></div></section><section class="section"><h2>Anlagenbild</h2><div class="card schema-card">${functionalSchema ? configuredHeatPumpSchema(report, functionalSchema) : heatPumpSchema(report, "anlage")}</div></section><section class="section"><h2>Viessmann-Komponentenbild</h2><div class="card schema-card">${productionSchema ? configuredHeatPumpSchema(report, productionSchema) : heatPumpSchema(report, "komponenten")}</div></section><section class="section"><h2>Aktuelle Heizkurve</h2><div class="card schema-card">${curve}</div></section><section class="section"><details class="card"><summary>Technische Rohdaten · ${report.features.length} Werte</summary><div class="feature-table"><table><thead><tr><th>Feature</th><th>Property</th><th>Wert</th><th>Einheit</th></tr></thead><tbody>${featureRows}</tbody></table></div></details></section>`;
}

async function renderHeatPump() {
  await renderHeatPumpLegacy();
  if (state.view !== "heat-pump" || !lastHeatPumpReport) return;
  const storageGrid = document.querySelector(".storage-history-grid");
  if (storageGrid) {
    storageGrid.classList.add("storage-history-grid--combined");
    storageGrid.innerHTML = `<div class="card dhw-history-card storage-history-combined">${combinedStorageHistoryChart(lastHeatPumpReport.dhw_history, lastHeatPumpReport.buffer_history)}</div>`;
  }
}

function combinedStorageHistoryChart(dhwHistory, bufferHistory) {
  const series = [
    { title: "Warmwasserspeicher", history: dhwHistory || [], color: "#d97706" },
    { title: "Pufferspeicher", history: bufferHistory || [], color: "#2563eb" },
  ].map((item) => ({
    ...item,
    points: item.history.map((point) => ({ time: new Date(point.recorded_at), value: Number(point.celsius) })).filter((point) => Number.isFinite(point.value) && !Number.isNaN(point.time.getTime())),
  }));
  const points = series.flatMap((item) => item.points);
  if (points.length < 2) return '<div class="empty">Noch keine ausreichende Speicherhistorie verfügbar.</div>';
  const width = 900; const height = 330; const left = 52; const right = 24; const top = 32; const bottom = 48;
  const minimum = Math.floor(Math.min(...points.map((point) => point.value)) - 1);
  const maximum = Math.ceil(Math.max(...points.map((point) => point.value)) + 1);
  const start = Math.min(...points.map((point) => point.time.getTime()));
  const end = Math.max(...points.map((point) => point.time.getTime()));
  const x = (point) => left + (point.time.getTime() - start) * (width - left - right) / Math.max(1, end - start);
  ...
  const lines = series.map((item) => item.points.length < 2 ? "" : `<polyline points="${item.points.map((point) => `${x(point).toFixed(1)},${y(point.value).toFixed(1)}`).join(" ")}" fill="none" stroke="${item.color}" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/>`).join("");
  const dots = series.map((item) => item.points.map((point) => `<circle cx="${x(point).toFixed(1)}" cy="${y(point.value).toFixed(1)}" r="2.5" fill="${item.color}"/>`).join("")).join("");
  const timeLabels = [...new Set(points.map((point) => point.time.getTime()))].sort((a, b) => a - b).filter((_, index, values) => index === 0 || index === values.length - 1 || index % Math.max(1, Math.floor(values.length / 5)) === 0).map((timestamp) => `<text x="${x({ time: new Date(timestamp) })}" y="${height - 16}" text-anchor="middle" fill="#627d98" font-size="10">${esc(new Date(timestamp).toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" }))}</text>`).join("");
  return `<div class="dhw-history"><div class="dhw-history__summary"><div><span>Warmwasser aktuell</span><strong>${series[0].points.length ? series[0].points.at(-1).value.toFixed(1) : "n/a"} °C</strong></div><div><span>Puffer aktuell</span><strong>${series[1].points.length ? series[1].points.at(-1).value.toFixed(1) : "n/a"} °C</strong></div><div><span>Messreihen</span><strong>2</strong></div></div><div class="dhw-history__chart"><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Temperaturverlauf von Warmwasserspeicher und Pufferspeicher über die letzten 24 Stunden">${grid}${lines}${dots}${timeLabels}<text x="${left}" y="15" fill="#52606d" font-size="11" font-weight="700">Speichertemperaturen · °C</text></svg></div><div class="combined-storage-legend"><span class="combined-storage-legend__dhw">Warmwasserspeicher</span><span class="combined-storage-legend__buffer">Pufferspeicher</span></div><div class="dhw-history__note">Archivierte Viessmann-Messwerte · Zeitraum: letzte 24 Stunden</div></div>`;
}
...
function configuredHeatPumpSchema(report, schema) {
  const featureValues = new Map(report.features.map((item) => [`${item.feature}.${item.property}`, `${item.value ?? "nicht verfügbar"}${item.unit ? ` ${item.unit}` : ""}`]));
  const nodeCenter = (item) => ({ x: item.x + item.width / 2, y: item.y + item.height / 2 });
  const typeLabel = (type) => ({ outdoor: "Außeneinheit", unit: "Inneneinheit", tank: "Speicher", dhw: "Warmwasser", circuit: "Heizkreis", sensor: "Messpunkt" })[type] || "Komponente";
  const formatValue = (raw) => { const value = String(raw).replace(/\s+celsius\b/gi, " °C").replace(/\s+percent\b/gi, " %").replace(/\btrue\b|\bon\b|\bactive\b/gi, "aktiv").replace(/\bfalse\b|\boff\b|\bready\b/gi, "bereit").replace(/\bstandby\b/gi, "Bereitschaft").replace(/\bheating\b/gi, "Heizen"); return value; };
  const edges = schema.edges.map((connection) => { const source = schema.nodes.find((item) => item.id === connection.source); const target = schema.nodes.find((item) => item.id === connection.target); if (!source || !target) return ""; const start = nodeCenter(source); const end = nodeCenter(target); return `<path d="M ${start.x} ${start.y} C ${(start.x + end.x) / 2} ${start.y}, ${(start.x + end.x) / 2} ${end.y}, ${end.x} ${end.y}"/>`; }).join("");
  const nodes = schema.nodes.map((item) => { const binding = item.binding || {}; const value = formatValue(featureValues.get(`${binding.feature}.${binding.property}`) || "nicht verfügbar"); const status = item.type === "outdoor" ? `Status: ${value}` : ""; const displayValue = status || value; const isSensor = item.type === "sensor"; const titleSize = Math.max(11, Math.min(isSensor ? 15 : 17, 230 / Math.max(1, item.label.length))); const shapeClass = item.type === "tank" || item.type === "dhw" ? "configured-node-shape configured-node-tank" : isSensor ? "configured-node-shape configured-node-sensor" : "configured-node-shape"; const shapeSize = isSensor ? 44 : 52; const valuePanelClass = isSensor ? "configured-node-value-panel configured-node-value-panel-sensor" : "configured-node-value-panel"; const valuePanel = `<rect class="${valuePanelClass}" x="${item.x + 10}" y="${item.y + item.height - 30}" width="${item.width - 20}" height="22" rx="5"/><text class="configured-node-value configured-node-value-large" text-anchor="middle" x="${item.x + item.width / 2}" y="${item.y + item.height - 14}">${esc(displayValue)}</text>`; return `<g class="configured-node${isSensor ? " configured-node-sensor-card" : ""}"><rect class="configured-node-body" x="${item.x}" y="${item.y}" width="${item.width}" height="${item.height}" rx="10"/><rect class="${shapeClass}" x="${item.x + 16}" y="${item.y + 16}" width="${shapeSize}" height="${shapeSize}" rx="${isSensor ? 22 : item.type === "tank" || item.type === "dhw" ? 24 : 6}"/><text class="configured-node-title" font-size="${titleSize}" x="${item.x + (isSensor ? 70 : 80)}" y="${item.y + 25}">${esc(item.label)}</text><text class="configured-node-type" x="${item.x + (isSensor ? 70 : 80)}" y="${item.y + 41}">${esc(typeLabel(item.type))}</text>${valuePanel}</g>`; }).join("");
  return `<svg viewBox="0 0 ${schema.viewport.width} ${schema.viewport.height}" role="img" aria-label="${esc(schema.name || "Wärmepumpenschema")}"><defs><marker id="configured-arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#6b8b93"/></marker></defs><g class="configured-edges" marker-end="url(#configured-arrow)">${edges}</g>${nodes}</svg>`;
}
...
function heatPumpSchema(report, variant) {
  ...
  const metrics = report.metrics;
  const value = (name) => { const raw = String(map[name]?.value || "nicht verfügbar").replace(" celsius", " °C").replace(" percent", " %"); return ({ heating: "Heizen", standby: "Bereitschaft", efficientwithmincomfort: "Komfortbetrieb", efficientWithMinComfort: "Komfortbetrieb", true: "Ja", false: "Nein" }[raw] || raw).replace(" °C", "°C").replace(" %", "%"); };
  ...
  if (variant === "anlage") {
    return `<svg viewBox="0 0 1280 640" role="img" aria-label="Produktionsnahes Heizungs- und Warmwasserschema"><defs><linearGradient id="buffer-clean" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#c51f1f"/><stop offset=".5" stop-color="#5b3d75"/><stop offset="1" stop-color="#1976d2"/></linearGradient></defs><g fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M245 260 H300 M490 300 H575 M735 330 H920" stroke="#a83b36" stroke-width="8"/><path d="M300 405 H245 M490 410 H575 M920 510 H810 V420 H735" stroke="#385b85" stroke-width="8"/><path d="M490 220 H535 V105 H575 M490 250 H550 V165 H575" stroke="#c98b4a" stroke-width="6"/><path d="M920 330 H1160 V365 H920 V400 H1160 V435 H920 V470 H1160 V505 H920" stroke="#a83b36" stroke-width="7"/><path d="M920 510 H1160" stroke="#385b85" stroke-width="7"/></g><g fill="#fff" stroke="#17202a" stroke-width="2"><rect x="35" y="170" width="210" height="245" rx="10"/><rect x="300" y="200" width="190" height="215" rx="10"/><rect x="575" y="255" width="160" height="190" rx="10" fill="url(#buffer-clean)"/><rect x="575" y="35" width="160" height="190" rx="10" fill="url(#buffer-clean)"/></g><g font-family="sans-serif" text-anchor="middle" fill="#17202a"><text x="140" y="205" font-weight="800">AUSSENEINHEIT</text><text x="140" y="230">${esc(report.model)}</text><text x="140" y="350">Verdichter ${active ? "aktiv" : "aus / bereit"}</text><text x="90" y="380">LÜFTER 1</text><text x="90" y="400" font-weight="700">${esc(value("Außenlüfter 1"))}</text><text x="190" y="380">LÜFTER 2</text><text x="190" y="400" font-weight="700">${esc(value("Außenlüfter 2"))}</text><text x="395" y="235" font-weight="800">INNENEINHEIT</text><text x="395" y="285">UMWÄLZPUMPE</text><text x="395" y="305" font-weight="700">${esc(value("Inneneinheit Pumpe"))}</text><text x="395" y="330">ZUHEIZER</text><text x="395" y="350">${rod ? "bereit / heizt nicht" : "gesperrt"}</text><text x="655" y="292" font-weight="800">PUFFERSPEICHER</text><text x="655" y="330">${esc(value("Pufferspeicher"))}</text><text x="655" y="72" font-weight="800">WARMWASSER</text><text x="655" y="95">SPEICHER</text><text x="655" y="130" font-weight="700">${esc(value("Warmwasser"))}</text><text x="830" y="205">WARMWASSER-ZIRKULATION</text><text x="830" y="225">${esc(value("Warmwasser-Zirkulation"))}</text><text x="1040" y="290" font-weight="800">HEIZKREIS 1</text><text x="1040" y="320">Vorlauf ${esc(value("Heizkreis 1 Vorlauf"))}</text><text x="1040" y="350">Anlagenrücklauf ${esc(value("Heizungsrücklauf"))}</text></g></svg>`;
  }
...
Promise.all([getJson("/api/v1/home"), getJson("/api/v1/rooms"), getJson("/api/v1/heat-pump/functional-schema").catch(() => null), getJson("/api/v1/heat-pump/schema").catch(() => null)]).then(([home, rooms, functional, schema]) => { state.home = home; state.rooms = rooms; functionalSchema = functional; productionSchema = schema; renderHeader(); render(); }).catch((error) => { document.querySelector("#freshness").textContent = `Migration API nicht erreichbar: ${error.message}`; });
