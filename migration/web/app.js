...
async function renderHeatPump() {
  ...
  const curve = `<svg viewBox="0 0 760 280" role="img" aria-label="Heizkurve mit Außentemperatur- und Vorlaufachsen"><g stroke="#e6edf3">${[-20,-10,0,10,20].map((_, index) => `<line x1="${80 + index * 140}" y1="30" x2="${80 + index * 140}" y2="225"/>`).join("")}${[20,30,40,50,60].map((_, index) => `<line x1="80" y1="${225 - index * 48.75}" x2="700" y2="${225 - index * 48.75}"/>`).join("")}</g><line x1="80" y1="225" x2="700" y2="225" stroke="#7b8794"/><line x1="80" y1="30" x2="80" y2="225" stroke="#7b8794"/><polyline points="80,45 220,78 360,112 500,158 600,190 700,210" fill="none" stroke="#c2410c" stroke-width="4"/><g fill="#627d98" font-size="11" text-anchor="middle">${[-20,-10,0,10,20].map((value, index) => `<text x="${80 + index * 140}" y="244">${value} °C</text>`).join("")}</g><g fill="#627d98" font-size="11" text-anchor="end">${[20,30,40,50,60].map((value, index) => `<text x="72" y="${229 - index * 48.75}">${value} °C</text>`).join("")}</g><text x="390" y="270" text-anchor="middle" fill="#52606d" font-size="12" font-weight="700">Außentemperatur (°C)</text><text x="15" y="130" text-anchor="middle" transform="rotate(-90 15 130)" fill="#52606d" font-size="12" font-weight="700">Heizkreis-Vorlauf (°C)</text><text x="100" y="20" fill="#52606d" font-size="12" font-weight="700">Heizkurve · Steigung ${esc(metrics.curve_slope ?? "n/a")} · Shift ${esc(metrics.curve_shift ?? "n/a")}</text></svg>`;
